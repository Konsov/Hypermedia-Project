...
exports.serviceIdEventGET = function(id) {
  return new Promise(function(resolve, reject) {
    ...
    var subquery_2 = sqlDb.select('id_event').from('service_presented').where('id_service','in',subquery_1);
    sqlDb.select('id','name','contact','text_presentation','starting_date','ending_date','image').from('event').where('id','in',subquery_2)
    .then(response => {
      resolve(response);
    })
    .catch(err => {
...
exports.serviceIdPersonGET = function(id) {
  return new Promise(function(resolve, reject) {
    var subquery_1 = sqlDb.select('id').from('service').where('id',id);
    var subquery_2 = sqlDb.select('id_person').from('person_involved').where('id_service','in',subquery_1);
    sqlDb.select('id','name','age','role','profession','short_description','email','image').from('person').where('id','in',subquery_2)
    .then(response => {
      resolve(response);
    })
    .catch(err => {
...
exports.serviceIdProjectGET = function(id) {
  return new Promise(function(resolve, reject) {
    var subquery_1 = sqlDb.select('id').from('service').where('id',id);
    var subquery_2 = sqlDb.select('id_project').from('project_related').where('id_service','in',subquery_1);
    sqlDb.select('id','name','status','description','starting_date','ending_date').from('project').where('id','in',subquery_2)
    .then(response => {
      resolve(response);
    })
    .catch(err => {
